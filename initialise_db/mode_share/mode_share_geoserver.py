...
def create_sa2_2023_view(workspace_name: str, data_store_name: str):
    create_datastore_layer(workspace_name, data_store_name, layer_name="sa2_2023")


def create_mode_share_flow_view(workspace_name: str, data_store_name: str):
...
def create_mode_share_2023_view(workspace_name: str, data_store_name: str):
    mode_share_layer_name = "mode_share_2023"
    ...
    mode_share_sql_query = f"""
        SELECT
            ...
            "Did_not_go_to_work_today",
            "Other",
            "Total_stated_-_main_means_of_travel_to_work,_by_usual_residence", as "Total_residents"
        FROM {mode_share_layer_name}
    """
    # Escape characters in SQL query so that it is valid Geoserver XML
    xml_escaped_sql = saxutils.escape(mode_share_sql_query, entities={r"'": "&apos;", "\n": "&#xd;"})
...
def initialise_geoserver_mode_share():
    ...
    create_sa2_2023_view(workspace_name, data_store_name)
    create_mode_share_flow_view(workspace_name, data_store_name)
    create_mode_share_2023_view(workspace_name, data_store_name)
